...
testdataset = datasets.MNIST(root='../data', train=False, download=False, transform=transforms)
...
writer_train = SummaryWriter(log_dir='./log/train_cnn')
writer_test = SummaryWriter(log_dir='./log/test_cnn')
...
for epoch in range(total_epoch):
    ...
    writer_train.add_graph(net, images)
    writer_train.add_image('mnist_images', make_grid(images, nrow=10))
    writer_train.add_histogram('conv1.bias', net.convs.conv1.bias, epoch + 1)
    writer_train.add_histogram('conv1.weight', net.convs.conv1.weight, epoch + 1)
    writer_train.add_histogram('conv2.bias', net.convs.conv2.bias, epoch + 1)
    writer_train.add_histogram('conv2.weight', net.convs.conv2.weight, epoch + 1)
    ...
    writer_test.add_image('mnist_images', make_grid(images, nrow=10))
